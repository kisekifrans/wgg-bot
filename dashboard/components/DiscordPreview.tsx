import type { EmbedConfig } from '@/lib/types';
import {
  applyPlaceholders,
  parseButtonEmoji,
  renderDiscordMarkdown,
} from '@/lib/discordMarkdown';

const BUTTON_STYLES: Record<string, string> = {
  Primary: 'discord-btn-primary',
  Secondary: 'discord-btn-secondary',
  Success: 'discord-btn-success',
  Danger: 'discord-btn-danger',
};

type Props = {
  embed: EmbedConfig;
  button?: { label: string; emoji?: string; style?: string };
  label?: string;
  variant?: 'panel' | 'welcome' | 'command';
  /** Sample values shown in welcome preview */
  welcomeMeta?: { ticketId?: string; category?: string };
  showPings?: boolean;
  placeholders?: { user?: string; staff?: string; username?: string };
  botName?: string;
};

export function DiscordPreview({
  embed,
  button,
  label = 'Preview',
  variant = 'panel',
  welcomeMeta,
  showPings,
  placeholders,
  botName = 'WGG Ticket',
}: Props) {
  const color = embed.color || '#5865F2';
  const title = applyPlaceholders(embed.title || '', placeholders);
  const description = applyPlaceholders(embed.description || '', placeholders);
  const footer = embed.footer || '';
  const btnStyle = BUTTON_STYLES[button?.style || 'Secondary'] || BUTTON_STYLES.Secondary;
  const btnEmoji = parseButtonEmoji(button?.emoji);

  const ticketId = welcomeMeta?.ticketId || 'wgg-ticket-0001';
  const category = welcomeMeta?.category || 'Account';

  return (
    <div className="discord-frame">
      <div className="discord-frame-header">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="#5865F2" aria-hidden>
          <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515a.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" />
        </svg>
        {label}
      </div>

      <div className="discord-message-body">
        {variant === 'welcome' && showPings && (
          <div className="discord-pings">
            <span className="discord-md-mention">@User</span>{' '}
            <span className="discord-md-mention discord-md-role">@Staff</span>
          </div>
        )}

        <div className="discord-message-row">
          <div className="discord-avatar" aria-hidden>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/wgg-icon.png"
              alt=""
              width={40}
              height={40}
              className="h-full w-full object-contain"
              draggable={false}
            />
          </div>
          <div className="min-w-0 flex-1">
            <div className="discord-author">
              <span className="discord-author-name">{botName}</span>
              <span className="discord-bot-tag">BOT</span>
              <span className="discord-timestamp">Today at 12:00 PM</span>
            </div>

            <div className="discord-embed-wrap">
              <div className="discord-embed" style={{ borderColor: color }}>
                {title && (
                  <div className="discord-embed-title">{renderDiscordMarkdown(title)}</div>
                )}
                {description ? (
                  <div className="discord-embed-desc">{renderDiscordMarkdown(description)}</div>
                ) : (
                  !title && variant !== 'welcome' && (
                    <div className="discord-embed-desc discord-embed-empty">No description yet</div>
                  )
                )}

                {variant === 'welcome' && (
                  <div className="discord-embed-fields">
                    <div className="discord-embed-field">
                      <div className="discord-embed-field-name">Ticket ID</div>
                      <div className="discord-embed-field-value">{ticketId.toUpperCase()}</div>
                    </div>
                    <div className="discord-embed-field">
                      <div className="discord-embed-field-name">Category</div>
                      <div className="discord-embed-field-value">{category}</div>
                    </div>
                    <div className="discord-embed-field">
                      <div className="discord-embed-field-name">Status</div>
                      <div className="discord-embed-field-value">🟢 Open</div>
                    </div>
                  </div>
                )}

                {footer && (
                  <div className="discord-embed-footer">
                    <span>{footer}</span>
                    <span className="discord-embed-footer-dot">•</span>
                    <span>Today at 12:00 PM</span>
                  </div>
                )}
              </div>
            </div>

            {button && (
              <div className="discord-actions">
                <span className={`discord-btn ${btnStyle}`}>
                  <span className={btnEmoji.isCustom ? 'discord-md-custom-emoji' : ''}>
                    {btnEmoji.display}
                  </span>
                  <span>{button.label || 'Create ticket'}</span>
                </span>
              </div>
            )}
          </div>
        </div>
      </div>
    </div>
  );
}
